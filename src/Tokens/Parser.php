...
namespace Coco\JsonParser\Tokens;

use Coco\JsonParser\Decoders\ConfigurableDecoder;
use Coco\JsonParser\Exceptions\SyntaxException;
use Coco\JsonParser\Tokens\CompoundBegin;
use Coco\JsonParser\Tokens\CompoundEnd;
use Coco\JsonParser\Tokens\Token;
use Coco\JsonParser\ValueObjects\Config;
use Coco\JsonParser\ValueObjects\State;
use Generator;
...
final class Parser implements IteratorAggregate
{
     ...
     *
     * @param Generator|Traversable $tokens
     * @param Config                $config
     */
    public function __construct(Generator|Traversable $tokens, Config $config)
    {
        $this->tokens = $tokens;
        $this->config = $config;
    ...
    {
        $state = new State($this->config->pointers, function () {
            new self($this->lazyLoad(), clone $this->config);
        });
